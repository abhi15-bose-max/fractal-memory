"""
experiments/drift_fast_comparison.py

Compares FractalModel vs ESN vs LSTM vs GRU vs MLP
on FAST-drifting Mackey–Glass data.

Run:
    python -m experiments.drift_fast_comparison
"""
...
# ----------------------
# Generate FAST drift MG
# ----------------------
print("Generating FAST drift MG...")
x = gen_fast_drift_mg(T=T, seed=SEED)
...
out = fm.run_sequence(x, consolidate_every=250, min_consolidate_samples=60)
...
# ----------------------
# 4) GRU baseline (fixed bug: use batch_first=True)
# ----------------------
...
class GRUNet(nn.Module):
    def __init__(self, win, hid):
        super().__init__()
        self.rnn = nn.GRU(1, hid, batch_first=True)  # <-- fixed here
        self.fc = nn.Linear(hid, 1)
...
# PLOTS
# ----------------------
print("Plotting FAST drift results...")
...
plt.legend()
plt.title("FAST Drift MG: Test Segment Predictions")
plt.tight_layout()
p1 = os.path.join(OUTDIR, "fast_drift_segment.png")
...
plt.bar(labels, vals)
plt.ylabel("MSE")
plt.title("FAST Drift MG: Model Comparison")
plt.tight_layout()
p2 = os.path.join(OUTDIR, "fast_drift_mse_bar.png")
