"""
experiments/drift_slow_comparison.py

Compares FractalModel vs ESN vs LSTM vs GRU vs MLP
on SLOW-drifting Mackey–Glass data.

Run:
    python -m experiments.drift_slow_comparison
"""
import os
import numpy as np
...
# ----------------------
# Generate SLOW drift MG
# ----------------------
print("Generating SLOW drift MG...")
x = gen_slow_drift_mg(T=T, seed=SEED)
...
# ----------------------
# 1) FRACTAL MODEL (favor slow consolidation)
# ----------------------
print("Running FractalModel (slow-favoring)...")
fm = FractalModel(
    res_size=RES_SIZE,
    reservoir_kwargs={"spectral_radius": 0.95, "leak": 0.25, "seed": SEED},
    buffer_maxlen=2000,      # large buffer to capture long history
    online_lr=1e-4,
    device=DEVICE,
)
# less frequent consolidation but using more samples when it happens
out = fm.run_sequence(x, consolidate_every=500, min_consolidate_samples=100)
...
# PLOT RESULTS
# ----------------------
print("Plotting SLOW drift results...")
...
plt.legend()
plt.title("SLOW Drift MG: Test Segment Predictions")
plt.tight_layout()
p1 = os.path.join(OUTDIR, "slow_drift_segment.png")
...
plt.bar(labels, vals)
plt.ylabel("MSE")
plt.title("SLOW Drift MG: Model Comparison")
plt.tight_layout()
p2 = os.path.join(OUTDIR, "slow_drift_mse_bar.png")
